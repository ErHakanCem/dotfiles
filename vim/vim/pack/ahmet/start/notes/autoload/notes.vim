function! notes#new(name) " {
  " Use global variable or fallback to current directory
  let notes_dir = exists('g:notes_directory') ? expand(g:notes_directory) : getcwd()
  ...
  " Create timestamp-based ID instead of UUID
  " Format: YYYYMMDD.HHMMSS  → 20251016.145512
  let id = strftime("%Y%m%d.%H%M%S")
  let file = notes_dir . '/' . id . '.md'

  " If filename already exists (unlikely), retry
...
function! notes#search_note() " {
  ...
  call fzf#vim#ag('^# ', '', {'sink': function('notes#link_handler'), 'options': ['--prompt', 'Notes> ']})
